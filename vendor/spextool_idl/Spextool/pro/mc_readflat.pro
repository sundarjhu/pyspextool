...
;     2016-06-29 - Updated fetch of ODR keywords to allow for 3 digit
;                  order numbers.
;-
...
;  Get hdr info.
  ...
  modename  = strtrim(fxpar(hdr, 'MODE'),2)
...
;  Now we have to check to see whether it is a pre-iSHELL header or
;  not.

  tmp = 'ODR'+string(orders[0],FORMAT='(i2.2)')+'_T*'
  val = fxpar(hdr,tmp,COUNT=count)
  ...
  for i = 0,norders-1 do begin
     ...
     name_T = prefix+string(orders[i],FORMAT=format)+'_T*'
     name_B = prefix+string(orders[i],FORMAT=format)+'_B*'
     ...
     name         = prefix+string(orders[i],FORMAT=format)+'_XR'
     xranges[*,i] = long( strsplit( fxpar(hdr,name), ',', /EXTRACT) )
     
     name = prefix+string(orders[i],FORMAT=format)+'RMS'
     rmss = fxpar(hdr,name)
     rms[i] = rmss
     ...
     if arg_present(omask) then begin
        ...
        for j = 0,xranges[1,i]-xranges[0,i] do begin
           ...
           omask[x[j],floor(botedge[j]):ceil(topedge[j])] = orders[i]       
