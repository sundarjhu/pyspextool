...
;     2016-06-29 - Modified ODR keywords to allow for 3 digit order numbers
;     2018-01-04 - Fixed bug where the last edgecoefficient
;                  wasn't being written to the file.
;     2019-03-29 - Was not writing the modename to disk.  Fixed.
;-
pro mc_writeflat,normflat,normvar,flag,hdrinfo,rotation,orders,edgecoeffs, $
                 xranges,ps,slith_pix,slith_arc,slitw_pix,slitw_arc, $
                 modename,rp,version,outname,LINCORMAX=lincormax,$
                 DARK=dark,RMS=rms,HISTORY=history,CANCEL=cancel
...
;  if cancel then return
;  cancel = mc_cpar('mc_writeflat',rp,15,'Resolving Power',[2,3,4,5],0)
;  if cancel then return
;  cancel = mc_cpar('mc_writeflat',version,16,'Version',7,0)
;  if cancel then return
;  cancel = mc_cpar('mc_writeflat',outname,17,'Outname',7,0)
;  if cancel then return
...
;  Make hdr with hdrinfo in it.
  ...
  for i = 0, ntags-1 do begin
     ...
     sxaddpar,hdr,names[i],(hdrinfo.vals.(i)),(hdrinfo.coms.(i)), $
              BEFORE='COMMENT'
     
  endfor

  sxaddpar,hdr,'MODE',modename,' Instrument Mode'
  sxaddpar,hdr,'NORDERS',norders, ' Number of orders identified', $
           BEFORE='COMMENT'
  fxaddpar,hdr,'ORDERS',strjoin(strcompress(orders,/re),','), $
           ' Orders identified'
  sxaddpar,hdr,'PLTSCALE',ps, ' Plate scale (arcseconds per pixel)'
  sxaddpar,hdr,'SLTH_PIX',slith_pix, ' Slit length in pixels'
  sxaddpar,hdr,'SLTH_ARC',slith_arc, ' Slit length in arcseconds'
  sxaddpar,hdr,'SLTW_PIX',slitw_pix, ' Slit width in pixels'
  sxaddpar,hdr,'SLTW_ARC',slitw_arc, ' Slit width in arcseconds'
  sxaddpar,hdr,'RP',long(rp),' Resolving power'
  sxaddpar,hdr,'ROTATION',rotation,  ' IDL ROTATE value'
  sxaddpar,hdr,'VERSION',version, ' Spextool version'

  if n_elements(LINCORMAX) ne 0 then begin

     sxaddpar,hdr,'LINCRMAX',lincormax, ' Linearity maximum (DN)'
     
  endif
  
  if n_elements(dark) eq 0 then dark = 'None'
  sxaddpar,hdr,'DARK',dark,' Dark frame'
...
;  Now add edge coeff
  ...
  sxaddpar,hdr,'EDGEDEG',fitorder-1, $
     ...
     for j = 0, 1 do begin
        ...
        for k = 0, fitorder-1 do begin
           ...
           if j eq 0 then begin
              ...
              name    = 'OR'+string(orders[i],FORMAT='(i3.3)') + $
