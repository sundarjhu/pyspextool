...
;
; CALLING SEQUENCE:
;     result = mc_polyfit2d(x,y,z,xorder,yorder,ZERR=zerr,THRESHSVD=threshsvd,$
...
;     2016-07-06 - zerr -> zzerr in the construction of the A matrix.
;-
function mc_polyfit2d,x,y,z,xorder,yorder,ZERR=zerr,THRESHSVD=threshsvd,$
                      JUSTFIT=justfit,SILENT=silent,DOALPHA=doalpha,$
                      COVAR=covar,ZFIT=zfit,RMS=rms,CHISQ=chisq,$
                      RCHISQ=rchisq,Q=q,SIGMA=sigma,CANCEL=cancel
...
if n_params() lt 5 then begin
    ...
    print, 'Syntax - coeff = mc_polyfit2d(x,y,z,xorder,yorder,ZERR=zerr,$'
...
cancel = mc_cpar('mc_polyfit2d',x,1,'X',[2,3,4,5],1)
if cancel then return,-1
cancel = mc_cpar('mc_polyfit2d',y,2,'Y',[2,3,4,5],1)
if cancel then return,-1
cancel = mc_cpar('mc_polyfit2d',z,3,'Z',[2,3,4,5],1)
if cancel then return,-1
cancel = mc_cpar('mc_polyfit2d',xorder,4,'Xorder',[2,3,4,5],0)
if cancel then return,-1
cancel = mc_cpar('mc_polyfit2d',yorder,5,'Yorder',[2,3,4,5],0)
if cancel then return,-1
...
;  Construct the design matrix A, then alpha and beta.
    ...
    for i = 0, ncoeffs-1 do begin
        ...
        AT[*,i] = xx^order[0,i]*yy^order[1,i]/zzerr
...
if not keyword_set(JUSTFIT) then begin
    ...
    zfit   = mc_poly2d(xx,yy,xorder,yorder,coeffs)
    rms    = stddev(zz-zfit)
