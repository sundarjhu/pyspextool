;+
; NAME:
;     mc_findpeaks
;
; PURPOSE:
;     Identifies the peaks in the spatial profile of a point source.
;
; CATEGORY:
;     Spectroscopy
;
; CALLING SEQUENCE:
;     mc_findpeaks,profiles,npeaks,doorders,positions,apsign,AUTO=auto,$
;               GUESS=guess,FIXED=fixed,CANCEL=cancel
;
; INPUTS:
;     profiles - An array [norders] of structures where 
;                struc = [[arcseconds],[data]]
;     npeaks   - Number of peaks to find
;     doorders - An array [norders] of 1s and 0s.  If 0, the mc_findpeaks
;                skips the order
;     
; OPTIONAL INPUTS:
;     None
;
; KEYWORD PARAMETERS:
;     AUTO   - Automatically find the peaks
;     GUESS  - An array [naps,norders] of the guess positions in units of x
;     FIXED  - An array [naps,norders] of the fixed positions in units of x
;     CANCEL - Set on return if there is a problem
;     
; OUTPUTS:
;     positions - An array with the positions of the peaks
;     apsign    - An array with +1 if the peak is ``positive'' and -1 if
;                 the peak is ``negative''.
;
; OPTIONAL OUTPUTS:
;     None
;
; COMMON BLOCKS:
;     None
;
; SIDE EFFECTS:
;     None
;
; RESTRICTIONS:
;     None
;
; PROCEDURE:
;     Three modes: 1)  Automatically finds the peaks by fitting
;     gaussian around the maximum pixel in abs(profiles),
;     subtracting the resulting fit off and itterating until npeaks
;     peaks are found.  2) Manually will fit gaussians to the guess
;     positions.  3) Simply fixes the apertures to the guess positions.
;
; EXAMPLE:
;     
; MODIFICATION HISTORY:
;     2000-04-11 - Written by M. Cushing, Institute for Astronomy, UH
;     2001-01-09 - Fixed bug which would not allow the program to work
;                  on a single order.
;     2002-11-20 - Modified profiles structure in accordance with 
;                  new spextool definition.
;     2004-03-31 - Modified to remove NaNs.
;     2005-07-01 - Added gaussian subtraction to GUESS mode
;     2006-06-09 - Modified definition of guess and fixed inputs so
;                  that each order has a guess or fixed set of values
;-
pro mc_findpeaks,profiles,npeaks,doorders,positions,apsign,AUTO=auto, $
                 GUESS=guess,FIXED=fixed,CANCEL=cancel

  cancel = 0

  if n_params() lt 3 then begin
     
     print, 'Syntax - mc_findpeaks,profiles,npeaks,doorders,positions,apsign,$'
     print, '                      AUTO=auto,GUESS=guess,FIXED=fixed,$'
     print, '                      CANCEL=cancel'
     cancel = 1
     return
     
  endif
  cancel = mc_cpar('mc_findpeaks',profiles,1,'Profiles',8,[0,1])
  if cancel then return
  cancel = mc_cpar('mc_findpeaks',npeaks,2,'Npeaks',[2,3,4,5],0)
  if cancel then return
  cancel = mc_cpar('mc_findpeaks',doorders,3,'Doorders',[2,3,4,5],[0,1])
  if cancel then return
  
  norders = n_tags(profiles)

; Get the arrays set up
     
     positions = make_array(npeaks,norders,/FLOAT,VALUE=!values.f_nan)
     apsign    = make_array(npeaks,norders,/INT,VALUE=0)
  
  if n_elements(GUESS) ne 0 then begin
     
     for i = 0, norders-1 do begin

        if doorders[i] eq 0 then goto, cont1
        profile = profiles.(i)
        x = profile[*,0]
        y = profile[*,1]        

;  Clean NaNs

        idx = mc_nantrim(y,3,CANCEL=cancel)
        if cancel then return
        x = x[idx]
        y = y[idx]

;  Subtract "background"
        
        medy = median(y,/EVEN)
        y = y-medy
        ny = n_elements(y)

        for j =0,npeaks-1 do begin

;  Find guess position
           
           tabinv, x, guess[j,i], xpix

;  Fit gusssian
           
           result = mpfitpeak(double(x),double(y),fit,NTERMS=4, $
                              ESTIMATES=double([y[xpix],guess[j,i],0.5,0]))
           positions[j,i] = fit[1]

;  Now check apsign (zero ok because you have subtracted medy)
           
           tabinv, x, fit[1], xpix
           apsign[j,i] = (y[xpix] gt 0 ) ? 1:-1

;  Subtract and continue
           
           y = y - (fit[3]+gaussian(x,fit[0:2]))
           
        endfor
        cont1:
        
     endfor
     
  endif

  if keyword_set(AUTO) then begin

     for i = 0, norders-1 do begin
        
        if doorders[i] eq 0 then goto, cont2
        profile = profiles.(i)
        x = profile[*,0]
        y = profile[*,1]

;  Clean NaNs
        
        idx = mc_nantrim(y,3,CANCEL=cancel)
        if cancel then return
        x = x[idx]
        y = y[idx]

;  Subtract "background"
        
        medy = median(y,/EVEN)
        absy = abs(y-medy)
        ny = n_elements(absy)
        
        for j = 0, npeaks-1 do begin

;  Find guess position
           
           max = max(absy,xmax)

;  Fit gaussian

           result = mpfitpeak(double(x),double(absy),fit,NTERMS=4, $
                              ESTIMATES=double([max,x[xmax],0.5,0]))
           positions[j,i] = fit[1]

;  Now check apsign
           
           tabinv, x, fit[1], xpix
           apsign[j,i] = (y[xpix] gt medy ) ? 1:-1

;  Subtract and continue
           
           absy = absy - (fit[3]+gaussian(x,fit[0:2]))
           
           
        endfor
        cont2:
        
     endfor
     
  endif
  
  if n_elements(FIXED) ne 0 then begin

     for i = 0, norders-1 do begin
        
        if doorders[i] eq 0 then goto, cont3
        profile = profiles.(i)
        x = profile[*,0]
        y = profile[*,1]

;  Clean NaNs
        
        idx = mc_nantrim(y,3,CANCEL=cancel)
        if cancel then return
        x = x[idx]
        y = y[idx]
        
        medy = median(y,/EVEN)
        
        for j = 0, npeaks-1 do begin
           
           positions[j,i] = fixed[j,i]
           tabinv, x, fixed[j,i], xpix
           apsign[j,i] = (y[xpix] gt medy ) ? 1:-1
           
        endfor
        cont3:
        
     endfor
     
  endif
  
  for i = 0, norders-1 do begin
     
     if doorders[i] eq 1 then begin
        
        order = sort(positions[*,i])
        positions[*,i] = positions[order,i]
        apsign[*,i]  = apsign[order,i]
        
     endif
     
  endfor
  
end





