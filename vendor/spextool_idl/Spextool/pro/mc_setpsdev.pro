...
pro mc_setpsdev,filename,xsize,ysize,CLOSE=close,LANDSCAPE=landscape, $
  ...
  common info,fname,odevice,olandscape,encapsulated
  ...
  if keyword_set(CLOSE) then begin
     ...
     device, /CLOSE
     device, ENCAP=0,LANDSCAPE=0
     set_plot,odevice
     
     if keyword_set(CONVERT) then begin

;  Test to see if ghostscript is installed

        spawn, 'gs -version', result, error
        ...
        case encapsulated of
           ...
           1: begin
              ...
              if olandscape then begin

                 command = 'gs -dEPSCrop -sDEVICE=pdfwrite -sOutputFile='+ $
                           oname+' -dNOPAUSE -q -dBATCH -c' + $
                           ' "<</Orientation 2>> setpagedevice" -f "'+ $
                           fname+'"'

              endif else begin

                 command = 'gs -dEPSCrop -sDEVICE=pdfwrite -sOutputFile='+ $
                           oname+' -dBATCH -dNOPAUSE -f "'+fname+'"'
           ...
           0: begin
              ...
              if olandscape then begin
                 ...
                 command = 'gs -sDEVICE=pdfwrite -sOutputFile='+oname+ $
                           ' -dNOPAUSE -q -dBATCH -c "<</Orientation 2>> ' + $
                           'setpagedevice" -f "'+fname+'"'

              endif else begin

                 command = 'gs -sDEVICE=pdfwrite -sOutputFile='+oname+ $
                           ' -dNOPAUSE -q -dBATCH -f "'+fname+'"'
     ...
     fname = filename
     olandscape = keyword_set(LANDSCAPE)

     encapsulated = (strlen(fname)-strpos(fname,'.',/REVERSE_SEARCH) eq 4) ? 1:0
     ...
     device, FILENAME=filename,LANDSCAPE=landscape,XSIZE=xsize,YSIZE=ysize,$
             ...
             /HELVETICA,/ISOLATIN1,FONT_SIZE=font_size,ENCAP=encapsulated
