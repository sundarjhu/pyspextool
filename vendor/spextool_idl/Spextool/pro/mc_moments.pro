...
;                  output.
;     2017-08-24 - Fixed major bug where it was not reporting the
;                  ogoodbad array correctly...  Sigh.  
;-
...
;  Check for an input goodbad array.
  ...
  iigoodbad = keyword_set(IGOODBAD) ? igoodbad:replicate(1,n_elements(data))  

;  Check for NaNs

  nan = where(finite(data) eq 0, cnt_nan)    
  if cnt_nan ne 0 then iigoodbad[nan] = 2
  
;  Find good data.

  zinitgood = where(iigoodbad eq 1,count_initgood)
...
;    endif else begin
     ...
     med = median(data[zinitgood],/EVEN)
     mad = 1.482*median(abs(data[zinitgood]-med), /EVEN)
     
;    endelse
     
     zgood = where(abs( (data[zinitgood]-med)/mad ) le robust,cntgood, $
                   COMP=zbad,NCOMP=cntbad)
     
  endif else begin

     zgood = lindgen(count_initgood)
     cntbad = 0
...
;; endif else begin
;
  result = moment(data[zinitgood[zgood]],/NAN,DOUBLE=double)
;
;; endelse
;
 mean     = result[0]
 var      = result[1]
 stddev   = sqrt(var)
 stderror = sqrt(var/n_elements(zgood))
 skew     = result[2]
 kurt     = result[3]

 if cntbad ne 0 then ogoodbad[zinitgood[zbad]] = 0
