...
;
; CALLING SEQUENCE:
;     mc_simwavecal2d,ncols,nrows,edgecoeffs,xranges,slith_arc,wavecal,$
;                     spatacal,indices,CANCEL=cancel
;
; INPUTS:
...
;     ds          - The plate scale in arcseconds per pixel
;
; OPTIONAL INPUTS:
;     none
;
...
;               position on the sky
;     indices - A structure with [norders] tags.  Each tag is a 3D
;               array where array[*,*,0] gives the array indices of
;               constant wavelength and angle in the zeroth order and
;               array[*,*,1] gives the y indices of the array 
;               of constant wavelength and angle.  
;
; OPTIONAL OUTPUTS:
;     None
;
...
;     column basis, it fills the wavecal and spatcal files.  The x
;     indices of the indices array are just the column numbers while
;     the y indices are determined by scaling the length of the slit
;     by slith_arc.
;
; EXAMPLES:
;
...
;     2017-09-10 - Modified to include the new rectification indices
;-
pro mc_simwavecal2d,ncols,nrows,edgecoeffs,xranges,slith_arc,ds,wavecal, $
                    spatcal,indices,CANCEL=cancel
  ...
  if n_params() lt 6 then begin
     ...
     print, '                         slith_arc,ds,wavecal,spatcal,indices,$'
     print, '                         CANCEL=cancel'
     cancel = 1
     return
  ...
  if cancel then return
  cancel = mc_cpar('mc_simwavecal2d',ds, 6,'Ds',[2,3,4,5],0)
  if cancel then return  
...
;  Now start the indices
     ...
     ix        = dblarr(numwave+1,nsgrid+1,/NOZERO)
     iy        = dblarr(numwave+1,nsgrid+1,/NOZERO)
     ix[1:*,*] = rebin(xgrid,numwave,nsgrid+1)
     ix[0,1:*] = sgrid
     iy[1:*,0] = xgrid
     iy[0,1:*] = sgrid
     ...
     for j =0,stop-start do begin
        ...
        wavecal[xgrid[j],floor(botedge[j]):ceil(topedge[j])] = xgrid[j]
        ...
        iy[j+1,1:*] = interpol(ypix,spix,sgrid)
           
     endfor

     data = [[[ix]],[[iy]]]
     tag = string(i)
     indices = (i eq 0) ? create_struct(tag,data): $
               create_struct(indices,tag,data)
