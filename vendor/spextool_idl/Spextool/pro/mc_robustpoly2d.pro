...
;
; CALLING SEQUENCE:
;     result = mc_robustpoly2d(x,y,z,xorder,yorder,thresh,eps,ZERR=zerr,$
...
;
; PROCEDURE:
;     Same as mc_polyfit2d except it itteratively removes outliers.
;     Very basic.
;
; EXAMPLE:
;
...
;     2017-10-07 - Changed name to mc_robustpoly1d
;-
function mc_robustpoly2d,x,y,z,xorder,yorder,thresh,eps,ZERR=zerr,$
                         IGOODBAD=igoodbad,SILENT=silent,$
                         OGOODBAD=ogoodbad,COVAR=covar,RMS=rms,$
  ...
  if n_params() lt 7 then begin
     ...
     print, 'Syntax - result = mc_robustpoly2d(x,y,z,xorder,yorder,thresh,eps,$'
  ...
  cancel = mc_cpar('mc_robustpoly2d',x,1,'X',[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',y,2,'Y',[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',z,3,'Z',[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',xorder,4,'Xorder',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',yorder,5,'Yorder',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',thresh,6,'Thresh',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly2d',eps,7,'Eps',[2,3,4,5],0)
  if cancel then return,-1
...
;  Do a first pass fit to the data.
  ...
  coeff = mc_polyfit2d(xx,yy,zz,xorder,yorder,ZERR=zzerr,/SILENT,COVAR=covar,$
                       ZFIT=zfit,RMS=rms,CHISQ=chisq,RCHISQ=rchisq,Q=q,$
                       SIGMA=sigma)
  
  test = zz - zfit
  med = median(test,/EVEN)
  mad = 1.482*median(abs(test-med))
  good = where(abs( (test-med)/mad ) le thresh,count_good)
...
;  If it doesn't find any outliers then go back, if not continue.
  ...
  repeat begin
     ...
     coeff = mc_polyfit2d(xx[good],yy[good],zz[good],xorder,yorder,$
                          ZERR=zzerr[good],SILENT=1,COVAR=covar,ZFIT=zfit,$
                          RMS=rms,CHISQ=chisq,RCHISQ=rchisq,Q=q,SIGMA=sigma)
     
     test = zz[good] - mc_poly2d(xx[good],yy[good],xorder,yorder,coeff)
     med = median(test,/EVEN)
     mad = 1.482*median(abs(test-med))
     if ((mad_old-mad)/mad_old) lt eps then goto, cont1

     test = zz-mc_poly2d(xx,yy,xorder,yorder,coeff)
     good = where(abs( (test-med)/mad ) le thresh,count_good)
