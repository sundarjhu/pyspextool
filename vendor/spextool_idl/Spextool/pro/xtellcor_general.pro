...
;  Read spectra and load data.
  ...
  if strmid(obj,strpos(obj,'.',/REVERSE_S)+1) eq 'fits' then begin
     
     mc_readspec,obj,ospec
     mc_readspec,std,sspec

     ospec[*,0] = ospec[*,0]*scale
     sspec[*,1] = sspec[*,1]*scale
...
pro xtellcor_general_getshift,state
  ...
  shift = xmc_findshifts(obj_wave,obj_flux,new_flux,*state.d.awave, $
