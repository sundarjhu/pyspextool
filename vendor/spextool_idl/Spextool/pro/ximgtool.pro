...
;  Get user inputs
  ...
  if n_elements(PLOTWINSIZE) eq 0 then plotwinsize = [700,512]	else begin

     screensize = get_screen_size()

     plotwinsize = [0.9*screensize[0] < plotwinsize[0] > 512, $
                    0.8*screensize[1] < plotwinsize[1] > 512]
...
;  DIVISORS
  ...
  state = {blankbuffer:blankbuffer,$
           ...
           sexagesimal:1,$           
           speccoords_spat:ptr_new(2),$
           speccoords_wave:ptr_new(2),$
           speccoords_ordr:ptr_new(2),$
           speccoords_wunits:'',$
           speccoords_wfmt:'',$
           speccoords_sunits:'',$
           speccoords_sfmt:'',$
...
;=============================================================================
;
pro ximgtool_chspeccoords

  common ximgtool_state

  s = size(*state.speccoords_wave,/DIMEN)
  
  for i = 0,4 do begin
     
     if state.loadedbuffer[i] eq 1 then begin

        if s[0] ne buffers.(i).ncols or s[1] ne buffers.(i).nrows then begin

           *state.speccoords_wave = 0B
           *state.speccoords_spat = 0B
           *state.speccoords_ordr = 0B
           state.spec_wunits = ''
...
; Now check for the speccoords length

  if state.speccoords_wunits ne '' then begin
     
     omax = max(*state.speccoords_ordr,/NAN)
     text = 'o: '+strtrim(string(omax),2)+', l: '+ $
            strtrim(string(1.0,FORMAT=state.speccoords_wfmt),2)+ $
            ' '+state.speccoords_wunits+', s: '+ $
            strtrim(string(1,FORMAT=state.speccoords_sfmt),2)+' '+ $
            state.speccoords_sunits
     
     window, XSIZE=state.plotwinsize[0],YSIZE=state.plotwinsize[1], $
             /FREE,/PIXMAP
     
     xyouts,0.5,0.5,text,FONT=0,WIDTH=maxwidth,/NORM
     wdelete,!d.window
     state.speccoordssize = maxwidth*state.plotwinsize[0]
...
;  Figure out size of labels, XYZ first 
     ...
     maxtext = 'X: '+strtrim(string(buffers.(state.buffer).ncols, $
               ...
               ',  Y: '+strtrim(string(buffers.(state.buffer).nrows $
                                       ,FORMAT='(f7.2)'),2)+$
               ',  Z: '+strtrim(string(imgmax,FORMAT='(G+-12.6)') ,2)+' '+ $
     ...
     mintext = 'X: '+strtrim(string(buffers.(state.buffer).ncols, $
               ...
               ',  Y: '+strtrim(string(buffers.(state.buffer).nrows $
                                       ,FORMAT='(f7.2)'),2)+$
               ',  Z: '+strtrim(string(imgmin,FORMAT='(G+-12.6)') ,2)+' '+ $
               buffers.(state.buffer).zunits

     window, XSIZE=state.plotwinsize[0],YSIZE=state.plotwinsize[1],/FREE,/PIXMAP
     
     xyouts,0.5,0.5,maxtext,FONT=0,WIDTH=maxwidth,CHARSIZE=10,/NORM
     xyouts,0.5,0.5,mintext,FONT=0,WIDTH=minwidth,CHARSIZE=10,/NORM
     
     wdelete,!d.window
     
     buffers.(state.buffer).infosize = (maxwidth > minwidth)* $
                                       state.plotwinsize[0]     
...
;  Now do the divisors label
     ...
     if (*state.divisors) ne '' then begin
        ...
        xyouts,0.5,0.5,text,FONT=0,WIDTH=width,/NORM
        wdelete,!d.window
        buffers.(state.buffer).divisorssize = width
...
;  Display the image
  ...
  if not keyword_set(NODISPLAY) then begin
     ...
     widget_control, state.ximgtool_base, $
                     TLB_SET_TITLE=buffers.(state.buffer).title
     ximgtool_plotupdate
     ximgtool_trackwin
     ximgtool_updatebuttons
  ...
  if state.panner then begin
     ...
     if aratio gt 1 then begin
        ...
        ur = [buffers.(state.buffer).cenpix[0]+ $
              buffers.(state.buffer).pixdisp[0]/2.+offset,$
              buffers.(state.buffer).cenpix[1]+ $
              buffers.(state.buffer).pixdisp[1]/2.]+0.5 < $
             (buffers.(state.buffer).ncols+0.5)
...
; Do XYZ first
  ...
  if xy[0] lt -0.5 or xy[0] gt buffers.(state.buffer).ncols-0.5 or $
     xy[1] lt -0.5 or xy[1] gt buffers.(state.buffer).nrows-0.5 then $
        z = !values.f_nan else $
  ...
  val = 'X: '+strtrim(string(state.cursorpos[0],FORMAT='(f7.2)'),2)+ $
        ...
        ',  Z: '+strtrim(string(z,FORMAT='(G+-12.6)') ,2)+' '+ $
        buffers.(state.buffer).zunits
  ...
  xyouts,state.plotwinsize[0]-12,16,val,/DEVICE,COLOR=5, $
         CHARSIZE=10,FONT=0,ALIGNMENT=1

;  Do spectral coordiates if requested

  if state.speccoords_wunits ne '' then begin
     
     if xy[0] lt -0.5 or xy[0] gt buffers.(state.buffer).ncols-0.5 or $
        xy[1] lt -0.5 or xy[1] gt buffers.(state.buffer).nrows-0.5 then begin
        ...
        w = (*state.speccoords_wave)[xy[0],xy[1]]
        s = (*state.speccoords_spat)[xy[0],xy[1]]
        o = (*state.speccoords_ordr)[xy[0],xy[1]]
                
     endelse

     val = 'o: '+strtrim(string(o),2)+', l: '+ $
           strtrim(string(w,FORMAT=state.speccoords_wfmt),2)+' '+ $
           state.speccoords_wunits+', s: '+ $
           strtrim(string(s,FORMAT=state.speccoords_sfmt),2)+' '+ $
           state.speccoords_sunits

     size = state.speccoordssize
     polyfill, [state.plotwinsize[0]-12-size,state.plotwinsize[0]-12-size,$
                state.plotwinsize[0]-12,state.plotwinsize[0]-12,$
                state.plotwinsize[0]-12-size],$
               [12,12+15,12+15,12,12]+18,COLOR=0,/DEVICE
     
     xyouts,state.plotwinsize[0]-12,16+18,val,/DEVICE,COLOR=5, $
            CHARSIZE=10,FONT=0,ALIGNMENT=1
...
pro ximgtool_cleanupevent,event
  ...
  ptr_free, state.speccoords_wave
  ptr_free, state.speccoords_spat
  ptr_free, state.speccoords_ordr
...
pro ximgtool,img1,img2,img3,img4,img5,$
             ...
             NODISPLAY=nodisplay,CLEARFRAMES=clearframes, $
             CLEARALLFRAMES=clearallframes,SPECCOORDS=speccoords,QUIT=quit, $
             XYCENTER=xycenter,CANCEL=cancel
  ...
  if xregistered('ximgtool') and keyword_set(CLEARALLFRAMES) then begin
     
     ximgtool_clearframes,[2,3,4,5],CANCEL=cancel
...
;  Load images
  ...
  if keyword_set(SPECCOORDS) then begin

     *state.speccoords_wave = speccoords.wave
     *state.speccoords_spat = speccoords.spat
     *state.speccoords_ordr = speccoords.ordr
     state.speccoords_wunits = speccoords.wunits
     state.speccoords_wfmt = speccoords.wfmt
     state.speccoords_sunits = speccoords.sunits
     state.speccoords_sfmt = speccoords.sfmt
     ximgtool_chspeccoords
