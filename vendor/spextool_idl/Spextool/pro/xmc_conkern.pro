...
;  Now fit the data
  ...
  *state.d.coeffs = mc_robustpoly1d(state.d.wave,state.d.flux,$
...
;  Create the same as a postscript if necesssary

     
;     mc_setpsdev,'NormLine.eps',6,4,/ENCAP,FONT=12
;
;     !x.thick=2
;     !y.thick=2
;     !p.thick=2
...
;                state.d.awave gt state.p.xrange1[0])
;     
;     plot, state.d.awave[z],state.d.atrans[z],COLOR=2,$
;           YRANGE=[0,1],YSTYLE=5,XSTYLE=5,XRANGE=state.p.xrange1,PSYM=10, $
;           FONT=0,POSITION=[0.15,0.15,0.92,0.95]
;     
;     ticks = string(findgen(11)*.1,FORMAT='(F3.1)')
;     axis,YAXIS=1,YTICKS=10,YTICKNAME=ticks,YMINOR=1,COLOR=2,FONT=0
;     
;     plot,state.d.wave,state.d.flux,/XSTY,YSTY=9,$
;          YRANGE=state.p.yrange1,XRANGE=state.p.xrange1,PSYM=10,$
;          XTITLE='Wavelength (!9m!Xm)',YTITLE='f (DN s!U-1!N)',/NOERASE,$
;           FONT=0,POSITION=[0.15,0.15,0.92,0.95]
;     
;;  Now plot the normalization region
;     
...
;           tabinv,state.d.wave,(*state.r.normreg)[0,i],idx
;           plots,[(*state.r.normreg)[0,i],(*state.r.normreg)[0,i]],$
;                 [base,state.d.flux[idx]],COLOR=4,LINESTYLE=2
;           
;        endif
;        if finite((*state.r.normreg)[1,i]) eq 1 then begin
;           
