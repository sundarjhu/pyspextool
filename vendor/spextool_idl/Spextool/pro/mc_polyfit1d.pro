...
;
; CALLING SEQUENCE:
;     result = mc_polyfit1d(x,y,order,SVD=svd,GAUSSJ=gaussj,YERR=yerr,$
...
;    2017-11-08 - Renamed to mc_polyfit1d
;-
function mc_polyfit1d,x,y,order,SVD=svd,GAUSSJ=gaussj,YERR=yerr, $
                      GOODBAD=goodbad,THRESHSVD=threshsvd,SILENT=silent, $
                      COVAR=covar,YFIT=yfit,RMS=rms,CHISQ=chisq, $
                      RCHISQ=rchisq,Q=q,VAR=var,JUSTFIT=justfit,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 3 then begin
     ...
     print, 'Syntax - coeff =  mc_polyfit1d(x,y,order,SVD=svd,GAUSSJ=gaussj,$'
     print, '                               YERR=yerr,THRESHSVD=threshsvd,$'
     print, '                               SILENT=silent,COVAR=covar,$'
  ...
  cancel = mc_cpar('mc_polyfit1d',x,1,'X',[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_polyfit2d',y,2,'Y',[2,3,4,5,12,13,14,15],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_polyfit2d',order,3,'Order',[2,3,4,5],0)
  if cancel then return,-1
...
;Find all the non-singular values, in svdfit of N.R.
     ...
     if sinvals ne 0 then begin
        ...
        w[small] = 0.
        print, 'mc_polyfit1d: Warning: '+strcompress(sinvals,/remove) + $
