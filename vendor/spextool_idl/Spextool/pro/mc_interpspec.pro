...
;     2018-05-15 - Fixed a all the underflow errors.
;-
...
;  Create output "c"lean y and yerror arrays.
  ...
  ndat = n_elements(cox)
  coy = make_array(ndat,/DOUBLE,VALUE=!values.f_nan)
  if doerror then coyerror = make_array(ndat,/DOUBLE,VALUE=!values.f_nan)
...
;  Compute errors if requested.
  ...
  if doerror then begin
     ...
     ciyvar = ciyerror^2
     tcovar = make_array(c_idx,/DOUBLE,VALUE=!values.f_nan)
