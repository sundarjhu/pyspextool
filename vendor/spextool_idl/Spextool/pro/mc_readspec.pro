...
pro mc_readspec,specname,spc,hdr,obsmode,start,stop,norders,naps,orders,$
                xunits,yunits,slith_pix,slith_arc,slitw_pix,slitw_arc,$
                rp,airmass,xtitle,ytitle,instr,xranges,program,HISTORY=history,$
...
;  Check parameters
  ...
  if n_params() lt 1 then begin
     ...
     print, '                     HISTORY=history,WIDGET_ID=widget_id,$'
     print, '                     CANCEL=cancel'
     cancel = 1
     return
...
;  Get hdr info.
  ...
  obsmode  = strcompress( fxpar(hdr, 'MODE'), /RE )
...
;  Get extraction ranges
  ...
  airmass = fxpar(hdr,'AM')
  if !err lt 0 then airmass = fxpar(hdr,'AVE_AM')
