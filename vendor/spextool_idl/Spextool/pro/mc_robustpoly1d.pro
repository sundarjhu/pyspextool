...
;
; CALLING SEQUENCE:
;     result = mc_robustpoly1d,x,y,order,thresh,eps,SVD=svd,GAUSSJ=gaussj,$
...
;
; PROCEDURE:
;     Same as mc_polyfit1d except it iteratively throws data points out.
;     Very basic.
;
; EXAMPLE:
;     
...
;     2017-10-08 - Changed name to mc_robustpoly1d
;-
function mc_robustpoly1d,x,y,order,thresh,eps,SVD=svd,GAUSSJ=gaussj,$
                         YERR=yerr,THRESHSVD=threshsvd,IGOODBAD=igoodbad,$
...
;  Check parameters
  ...
  if n_params() lt 5 then begin
     ...
     print, 'Syntax - coeff = mc_robustpoly1d(x,y,order,thresh,eps,SVD=svd,$'
  ...
  cancel = mc_cpar('mc_robustpoly1d',x,1,'X',[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly1d',y,2,'Y',[2,3,4,5,12,13,14,15],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly1d',order,3,'Order',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly1d',thresh,4,'Thresh',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_robustpoly1d',eps,5,'Eps',[2,3,4,5],0)
  if cancel then return,-1
...
;  Do a first pass fit to the data.
  ...
  coeff = mc_polyfit1d(xx,yy,order,SVD=svd,GAUSSJ=gaussj,YERR=yyerr,$
...
;  If it doesn't find any outliers then go back, if not continue.
  ...
  repeat begin
     ...
     sig_old  = sig
;     mad_old  = mad
     coeff = mc_polyfit1d(xx[good],yy[good],order,SVD=svd,GAUSSJ=gaussj,$
  ...
  if not keyword_set(SILENT) then begin
     ...
     for i = 0, order do begin
        ...
        string = 'Coeff #'+string(i,format='(i2.2)')+'   =  '+ $
                 strcompress(coeff[i],/re)+'  +/-  '+ $
                 strcompress(sqrt(var[i]), /re)
        print, string
