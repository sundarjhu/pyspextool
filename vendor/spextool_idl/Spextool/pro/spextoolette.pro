...
;  Now create wavelength calibration image
  ...
  for j = 0, norders-1 do begin
     ...
     botedge = mc_poly1d(x,edgecoeffs[*,0,j])
     topedge = mc_poly1d(x,edgecoeffs[*,1,j])
     dif = topedge-botedge
     ...
     coeff = mc_polyfit1d(x,result,1,/SILENT)
     disp[j] = coeff[1]
...
;     plot,shifts,rms,/XSTY,/YSTY,xrange=[-0.5,0.5]
     ...
     coeff = mc_polyfit1d(shifts[(minidx-del):(minidx+del)],$
