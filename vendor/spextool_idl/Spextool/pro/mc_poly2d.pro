...
;
; CALLING SEQUENCE:
;     result = mc_poly2d(x,y,xorder,yorder,coeffs,CANCEL=cancel)
;
; INPUTS:
...
;-
function mc_poly2d,x,y,xorder,yorder,coeffs,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 5 then begin
     ...
     cancel = 1
     print, 'Syntax -  result = mc_poly2d(x,y,xorder,yorder,coeffs,$'
     print, '                             CANCEL=cancel)'
     return, -1
     
  endif
  cancel = mc_cpar('mc_poly2d',x,1,'X',[2,3,4,5],[0,1])
  if cancel then return,-1
  cancel = mc_cpar('mc_poly2d',y,2,'Y',[2,3,4,5],[0,1])
  if cancel then return,-1
  cancel = mc_cpar('mc_poly2d',xorder,3,'Xorder',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_poly2d',yorder,4,'Yorder',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_poly2d',coeffs,5,'Coeffs',[2,3,4,5],1)
  if cancel then return,-1
  ...
  for i = 0L, long(ninvar-1) do $
     z[i] = total((xx[i]^xexp # yy[i]^yexp ) * coeffs)
