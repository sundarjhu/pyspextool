...
;     2017-03-07 - Improved debugging plotting.
;-
pro mc_findorders,image,guesspos,sranges,step,slith_pix,degree,bufpix,$
                  frac,comwidth,edgecoeffs,xranges,WID=wid, $
                  PLOTGUESS=plotguess,CANCEL=cancel
...
;  Debugging variables
  ...
  if debug then begin
     ...
     window, /FREE,YSIZE=screensize[1]*0.9
     mywid = !d.window
...
;  the edges are found.
     ...
     if n_elements(WID) ne 0 and not debug then begin
        ...
        plotsym,0,1,/FILL
        plots,[guesspos[0,i],guesspos[0,i]],[guesspos[1,i],guesspos[1,i]], $
              COLOR=6,PSYM=8,SYMSIZE=1.5
...
;  Determine COM of edges
       ...
       if debug and debugorder eq i then begin
          ...
          plot,row,fcol,/XSTY,/YSTY,POS=pos[*,0],TITLE=scols[k],$
               XRANGE=[y_guess-slith_pix[1],y_guess+slith_pix[1]],$
               XTITLE='Row Number'
          plots,[y_guess,y_guess],!y.crange,COLOR=3
          plots,!x.crange,[z_guess,z_guess],COLOR=2
          plots,!x.crange,[z_guess*frac,z_guess*frac],COLOR=2
          plots,[guessyt,guessyt],!y.crange,COLOR=2
          plots,[guessyb,guessyb],!y.crange,COLOR=2

          
          plot, row,rcol,/xsty,/ysty,TITLE=scols[k], $
                XRANGE=[y_guess-slith_pix[1],y_guess+slith_pix[1]],$
                POS=pos[*,1],/NOERASE,$
                XTITLE='Row Number'
          plots,[y_guess,y_guess],!y.crange,COLOR=3
