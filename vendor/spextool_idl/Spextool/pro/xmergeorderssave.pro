...
;  Get the spectra for the requested aperture

  idx = indgen(state.norders)*state.naps+state.ap
  *state.workspec = (*state.origspec)[*,*,idx]
...
; Get plot ranges 
  ...
  for i = 0, state.norders-1 do begin
     ...
     wav = reform((*state.workspec)[*,0,i])
     flx = reform((*state.workspec)[*,1,i])
     unc = reform((*state.workspec)[*,2,i])
...
;  Reset a bunch of arrays
  ...
  *state.snrmask = make_array(npixels,state.norders,/BYTE,VALUE=1)
  *state.usrmask = make_array(npixels,state.norders,/BYTE,VALUE=1)
...
pro xmergeorders_mergeorders
  ...
  merged_w = (*state.workspec)[*,0,0]
  merged_f = (*state.workspec)[*,1,0]
  merged_u = (*state.workspec)[*,2,0]
  merged_b = (*state.workspec)[*,3,0]

  zsnr = where((*state.snrmask)[*,0] eq 0,cntsnr)
  zusr = where((*state.usrmask)[*,0] eq 0,cntusr)
  ...
  for i = 1,state.norders-1 do begin
     ...
     tmp_w = (*state.workspec)[*,0,i]
     tmp_f = (*state.workspec)[*,1,i]
     tmp_u = (*state.workspec)[*,2,i]
     tmp_b = (*state.workspec)[*,3,i]
  ...
  *state.mergedspec = [[merged_w],[merged_f],[merged_u],[merged_b]]
...
;  Do the order by order plotting 
  ...
  if state.plottype eq 'Overlapped' then begin
     ...
     for i = 0, state.norders-1 do begin
        ...
        if cntsnr ne 0 then spectra[zsnr,1:2,i] = !values.f_nan
        if cntusr ne 0 then spectra[zusr,1:2,i] = !values.f_nan
        
        wave   = spectra[*,0,i]
        z      = where(finite(wave) eq 1)
        wave   = wave[z]
        cflux  = spectra[z,1,i]
        cerror = spectra[z,2,i]
        flag   = spectra[z,3,i]
...
;  Plot masks
        ...
        if cntsnr ne 0 then begin
           ...
           xy = convert_coord(spectra[zsnr,0,i],!y.crange[1],/DATA,/TO_DEVICE)
        ...
        if cntusr ne 0 then begin
           ...
           xy = convert_coord(spectra[zusr,0,i],!y.crange[1],/DATA,/TO_DEVICE)
  ...
  if state.plottype eq 'Merged' then begin
     ...
     case state.spectype of 
        ...
        'Uncertainty': spec = spectra[*,2]
       
        'S/N': spec = spectra[*,1]/spectra[*,2]
        
     endcase
     oplot,(*state.mergedspec)[*,0],spec,PSYM=10
...
pro xmergeorders_snmask
  ...
  lim = mc_cfld(state.snlimit_fld,7,CANCEL=cancel)
  ...
  if lim ne '' then begin
     ...
     for i = 0,state.norders-1 do begin
        ...
        z = where(snr lt lim,cnt)
        if cnt ne 0 then (*state.snrmask)[z,i] = 0
...
pro xmergeorders_writefile
  ...
  history = 'This merged spectrum was derived from aperture '+$
            strtrim(string(state.ap+1,FORMAT='(i2.2)'),2)+' of file '+ $
            strtrim(file_basename(ifile),2)+'.'
  
  lim = mc_cfld(state.snlimit_fld,7,CANCEL=cancel)
  if lim ne '' then begin

     history = history+'A S/N cut of '+strtrim(lim,2)+' was applied.'
     ...
     if count eq 0 then begin
        ...
        case state.cursormode of 
           ...
           'Mask':  begin
              ...
              z = where((*state.workspec)[*,0,state.cliporder] gt min and $
                        (*state.workspec)[*,0,state.cliporder] lt max,cnt)
              if cnt ne 0 then (*state.usrmask)[z,state.cliporder] = 0
              xmergeorders_mergeorders
              xmergeorders_plotspec
           ...
           'Undo': begin
              ...
              min = min(state.reg[0,*],MAX=max)
              z = where((*state.workspec)[*,0,state.cliporder] gt min and $
                        (*state.workspec)[*,0,state.cliporder] lt max and $
                        (*state.usrmask)[*,state.cliporder] eq 0,cnt)
              if cnt ne 0 then (*state.usrmask)[z,state.cliporder] = 1
              xmergeorders_mergeorders
              xmergeorders_plotspec
...
;=============================================================================
;
pro xmergeorderssave,filename,CANCEL=cancel
