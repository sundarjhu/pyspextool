...
;
; PURPOSE:
;     To correct for amplifier noise in an iSHELL image
;
; CALLING SEQUENCE:
;     result = mc_ishellampcor(img,CANCEL=cancel)
;
; INPUTS:
;     img - A 2048 x 2048 iSHELL image in the default orientation.
;
; OPTIONAL INPUTS:
;     None
;
...
;-
function mc_ishellampcor,img,IVAR=ivar,OVAR=ovar,SILENT=silent,CANCEL=cancel

  cancel = 0
  
  if n_params() ne 1 then begin

     print, 'Syntax - result = mc_ishellampcor(img,SILENT=silent,CANCEL=cancel)'
     cancel = 1
     return, -1

  endif

  cancel = mc_cpar('mc_ishellampcor',img,1,'img',[2,3,4,5],[2])
