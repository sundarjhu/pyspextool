...
;
; PURPOSE:
;     To create interpolation indices to rectity spectral orders
;
; CALLING SEQUENCE:
;     result = mc_mkinterpindxs(edgecoeffs,xranges,coeffs,slith_arc,ds,ystep,$
...
;     coeffs     - A structure with 3 or 6 tags.
;                  coeffs.c1coeffs = 
;     slith_arc  -
;     ds         - 
;     ystep      -
...
;-
function mc_mkrectindcs2d,edgecoeffs,xranges,coeffs,slith_arc,ds,ystep, $
                          XGRIDS=xgrids,SGRID=sgrid,UPDATE=update, $
                          WIDGET_ID=widget_id,CANCEL=cancel
...
;  Figure out line information

  c1coeffs     = coeffs.c1coeffs
  c1coeffsxdeg = coeffs.c1xdeg
  c1coeffsydeg = coeffs.c1ydeg
  ndeg         = 1
  
  if n_tags(coeffs) eq 6 then begin

     c2coeffs     = coeffs.c2coeffs
     c2coeffsxdeg = coeffs.c2xdeg
     c2coeffsydeg = coeffs.c2ydeg
     ndeg         = 2
...
;  Start the loop over the pixels
     ...
     for j = 0,nxgrid-1 do begin
        ...
        c1z = mc_poly2d(xgrid[j],midslit[j],c1coeffsxdeg,c1coeffsydeg,c1coeffs)
        ...
        if ndeg eq 2 then begin
           ...
           c2z = mc_poly2d(xgrid[j],midslit[j],c2coeffsxdeg,c2coeffsydeg, $
                           c2coeffs)
           c1z = mc_poly2d(xgrid[j],midslit[j],c1coeffsxdeg,c1coeffsydeg, $
                           c1coeffs)
