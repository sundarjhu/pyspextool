...
;               lineinfo.id      = line IDs (string)
;               lineinfo.wwin    = fit window (in units of swave)
;               lineinfo.fittype = fit type(0=gaussian, 1=Lotentzian,2=centroid)
;               lineinfo.nterms  = Number of terms (3=basic,
;                                  4=basic+constant,5=basic+line) 
;               lineinfo.xguess  = x positions of lines in wspec.
;               lineinfo.xwin    = The fit window in units of pixels.
...
;     2019-01-30: Added the ability to fit using centroids
;-
pro findlines_plotcen,x,y,lineinfo,zline,m1,fwhm,goodbad,title,FONT=font, $
...
;  Plot the fit
  ...
  plots,[lineinfo.xguess,lineinfo.xguess],!y.crange,COLOR=6
  plots,[m1,m1],!y.crange,COLOR=3
  plots,[m1+fwhm/2.,m1+fwhm/2.],!y.crange,COLOR=3,LINESTYLE=1
  plots,[m1-fwhm/2.,m1-fwhm/2.],!y.crange,COLOR=3,LINESTYLE=1
...
;  Determine fit type
     ...
     if centroid then begin
        ...
        med = median([y[zline[0]],y[zline[linecnt-1]]],/EVEN)

        m1 = double(total(x[zline]*(y[zline]-med),/DOUBLE)/ $
                    total(y[zline]-med,/DOUBLE) )

        m2 = double(total((x[zline]-m1)^2*(y[zline]-med),/DOUBLE)/ $
                    total(y[zline]-med,/DOUBLE) )                

        linterp,x[zline],y[zline],m1,inten

        linten[i] = inten
        lxpos[i] = m1
        lfwhm[i] = sqrt(m2)*2.354

        if m1 le lineinfo[i].xguess+xthresh and $
           m1 ge lineinfo[i].xguess-xthresh and $
           m2 gt 0.0 and inten gt 0 then lgoodbad[i]  = 1     

        if plotsum ge 1 then begin
           
           findlines_plotcen,x,y,lineinfo[i],zline,m1,sqrt(m2)*2.354, $
                             lgoodbad[i],'Order '+strtrim(lineinfo[i].order,2)+$
...
;  line region.
        ...
        yfit = mpfitpeak(x[zline],y[zline]-offset,a,NTERMS=lineinfo[i].nterms,$
                         GAUSSIAN=gaussian,LORENTZIAN=lorentzian,/NAN)
